...
# Define dataset prefix for file naming (consistent with your original files)
dataset_prefix <- "Results_typical_cases_typical_concise_selected"

# Construct file paths for Stage 1 and Stage 2 data
...
# Combine Stage 1 and Stage 2 data, adding a stage identifier
all_data <- bind_rows(
...
ranked_models_by_points <- model_scores_summary %>%
  ...
  # Handle cases where a model might only have stage 1 or stage 2 data (fill with 0 for point calculation)
  mutate(
...
layered_plot_data <- model_scores_long %>%
  ...
  mutate(
    segment_start = ifelse(stage_numeric == 1, 0, lag(score, default = 0)),
    segment_height = ifelse(stage_numeric == 1, score, score - lag(score, default = 0))
  ) %>%
  ungroup() %>%
...
p <- ggplot() +
  ...
  geom_errorbar(
    data = model_scores_long_for_plot %>% filter(stage_numeric == 2),
    aes(
  ...
  scale_fill_manual(
    ...
    values = c("Stage 1" = "#4DBBD5", "Stage 2" = "#E64B35")
  ) +
  scale_x_continuous(
  ...
  labs(
    title = "Model Performance Comparison (Stages 1 & 2) - Common Cases",
    subtitle = "Models Ranked by Weighted Performance Score | K1, K5, K10 Accuracy Side-by-Side", # Updated subtitle
    y = "Accuracy Score"
  )
...
# --- 8. Save Outputs ---
plot_filename_base <- file.path(output_dir, "model_performance_stage1_stage2_top_k_weighted_rank") # New filename
ggsave(paste0(plot_filename_base, ".pdf"), plot = p, width = 14, height = 6, dpi = 300)
ggsave(paste0(plot_filename_base, ".png"), plot = p, width = 14, height = 6, dpi = 300)
...
table_filename <- file.path(output_dir, "model_performance_stage1_stage2_top_k_table_weighted_rank.csv") # New filename
...
# To show the points, we need to re-calculate or join with the points data
# For simplicity, just printing the top 5 models from the ranked list
